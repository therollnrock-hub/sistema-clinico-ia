...
def analizar_sintomas_nlp(texto: str):
    ...
    if any(p in texto_lower for p in criticos):
        score = 3
        especialidad = "Urgencias / Cardiología / UCI (Emergency / Cardiology)"
    elif any(p in texto_lower for p in moderados):
        score = 2
        especialidad = "Medicina Interna / Especialidad (Internal Medicine)"
        
    if "corazón" in texto_lower or "heart" in texto_lower: especialidad = "Cardiología / Cardiology"
    elif "hueso" in texto_lower or "bone" in texto_lower: especialidad = "Traumatología / Orthopedics"
    elif "cabeza" in texto_lower or "head" in texto_lower: especialidad = "Neurología / Neurology"

    niveles = {
        1: ("LEVE / RUTINARIO (MILD / ROUTINE)", "VERDE"),
        2: ("MODERADO / OBSERVACIÓN (MODERATE)", "AMARILLO"),
        3: ("CRÍTICO / ATENCIÓN INMEDIATA (CRITICAL)", "ROJO")
    }
    urgencia_txt, color = niveles[score]
    return {
        "urgencia": urgencia_txt,
        "nivel_color": color,
        "especialidad_sugerida": especialidad,
        "recomendacion_es": "Derivar de inmediato a urgencias." if score == 3 else "Programar cita prioritaria." if score == 2 else "Control rutinario.",
        "recomendacion_en": "Derive immediately to emergency." if score == 3 else "Schedule priority appointment." if score == 2 else "Routine control."
    }

# ==========================================
# 4. FASTAPI & SEGURIDAD
# ==========================================
app = FastAPI(title="Clinical Intelligence Center", version="5.0.0")
...
def home():
    ...
    <head>
        ...
        <title>Clinical Intelligence Center</title>
        <style>
            ...
            h1 { color: #2c3e50; text-align: center; margin-bottom: 5px; } .subtitle { text-align: center; color: #7f8c8d; margin-bottom: 25px; font-size: 14px; }
            .tabs { display: flex; border-bottom: 2px solid #ddd; margin-bottom: 25px; overflow-x: auto; }
            .tab-btn { flex: 1; padding: 12px; background: none; border: none; font-size: 15px; font-weight: bold; cursor: pointer; color: #7f8c8d; white-space: nowrap; }
            .tab-btn.active { color: #007bff; border-bottom: 3px solid #007bff; margin-bottom: -2px; }
            .tab-content { display: none; } .tab-content.active { display: block; }
            .form-group { margin-bottom: 20px; } label { display: block; font-weight: bold; margin-bottom: 8px; color: #34495e; }
            ...
            .faq-section { background: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 15px; border-left: 4px solid #007bff; }
            
            /* TARJETA DEL HIPERVÍNCULO A RODILLA */
            .knee-link-card { margin-top: 35px; padding: 20px; background: #2c3e50; color: white; border-radius: 10px; text-align: center; }
            .knee-btn { display: inline-block; margin-top: 10px; padding: 12px 25px; background: #e67e22; color: white; text-decoration: none; border-radius: 6px; font-weight: bold; transition: 0.3s; }
            .knee-btn:hover { background: #d35400; }
        </style>
    </head>
    <body>
        <div class="container">
            <div class="lang-switcher"><button class="lang-btn" onclick="toggleLanguage()" id="langBtn">🇬🇧 EN / 🇪🇸 ES</button></div>
            ...
            <div class="form-group" style="background: #f8f9fa; padding: 15px; border-radius: 8px;">
                <label for="token">🔑 Token de Autorización (Bearer Token):</label>
            ...
            <div id="tab-tabular" class="tab-content active">
                <div class="form-group">
                    ...
                    <select id="patologia" onchange="actualizarPlaceholder()">
                        <option value="diabetes">Diabetes Tipo 2 (8 variables)</option>
                        <option value="cardio">Riesgo Cardíaco (13 variables)</option>
                        <option value="oncologia">Oncología Mamaria (30 variables)</option>
                    </select>
                </div>
                <div class="form-group">
                    <label id="lbl-vars">Vector de Variables (Separadas por comas):</label>
            ...
            <div id="tab-nlp" class="tab-content">
                <div class="form-group">
                    <label id="lbl-sintomas">Describe los síntomas (Español o Inglés):</label>
                ...
                <div id="resultadoTriaje" class="result-card">
                    <div id="badge-urgencia" style="padding:10px; border-radius:6px; font-weight:bold; margin-bottom:10px;"></div>
            ...
            <div id="tab-faq" class="tab-content">
                <div id="faq-es">
                    <h2>📌 FAQ: Marco Teórico & Vectores</h2>
                    <div class="faq-section">
                        <h4>1. ¿Qué es un "Vector de Variables"?</h4>
                        <p>Representa el perfil fisiológico de un paciente en coordenadas numéricas dentro de un espacio multidimensional.</p>
                    </div>
                    <div class="faq-section">
                        <h4>2. El Evangelio de los Logits</h4>
                        <p>La red calcula el producto punto <i>z = (W &middot; X) + b</i> obteniendo el logit crudo. Luego, mediante la función Sigmoide <i>1 / (1 + e^-z)</i>, se mapea el riesgo entre 0% y 100%.</p>
                    </div>
                    <div class="faq-section">
                        <h4>3. Dimensiones por Patología</h4>
                        <ul>
                            <li><strong>Diabetes (8 dim):</strong> Glucosa, Presión, Edad, IMC, Insulina, etc.</li>
                            <li><strong>Cardiovascular (13 dim):</strong> Presión reposo, ECG, Colesterol, Frecuencia máxima, etc.</li>
                            <li><strong>Oncología (30 dim):</strong> Métricas celulares de biopsia (Radio, Perímetro, Concavidad, etc.).</li>
                        </ul>
                    </div>
                </div>
                <div id="faq-en" style="display:none;">
                    <h2>📌 FAQ: Theoretical Framework & Vectors</h2>
                    <div class="faq-section">
                        <h4>1. What is a "Variable Vector"?</h4>
                        <p>It represents the physiological profile of a patient as numerical coordinates in a multidimensional space.</p>
                    </div>
                    <div class="faq-section">
                        <h4>2. Disease Dimensions</h4>
                        <ul>
                            <li><strong>Diabetes (8 dim):</strong> Glucose, Blood Pressure, Age, BMI, Insulin, etc.</li>
                            <li><strong>Cardiovascular (13 dim):</strong> Resting BP, ECG, Cholesterol, Max HR, etc.</li>
                            <li><strong>Oncology (30 dim):</strong> Cell nucleus metrics from biopsy (Radius, Perimeter, Concavity, etc.).</li>
                        </ul>
                    </div>
                </div>
            </div>

            <!-- HIPERVÍNCULO DIRECTO A LA SEGUNDA APP (RODILLA) EN RENDER -->
            <div class="knee-link-card">
                <h3 style="margin-top:0; color:#3498db;">🦴 Módulo de Analítica DICOM & CNN (Rodilla)</h3>
                <p style="font-size: 14px; margin-bottom: 10px;">Accede al entorno independiente para procesar resonancias magnéticas (.dcm), visualizar imágenes y exportar reportes en CSV.</p>
                <!-- REEMPLAZA ESTA URL CON TU URL REAL DE RENDER PARA LA APP DE RODILLA -->
                <a href="https://app-rodilla.onrender.com" target="_blank" class="knee-btn" id="knee-link">
                    Abrir Servidor DICOM Rodilla ↗
                </a>
            </div>
        ...
        <script>
            ...
            async function enviarTabular() {
                ...
                const res = await fetch('/predecir/tabular', {
                    method: 'POST',
            ...
            async function enviarTriaje() {
                ...
                const res = await fetch('/triaje/sintomas', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                ...
                badge.innerText = "URGENCIA: " + data.urgencia;
            ...
            function toggleLanguage() {
                ...
                document.getElementById('langBtn').innerText = currentLang === 'es' ? "🇬🇧 EN / 🇪🇸 ES" : "🇪🇸 ES / 🇬🇧 EN";
            }
        </script>
    </body>
    </html>
...
def predecir_tabular(datos: PeticionTabular, token: str = Depends(verificar_token)):
    modelo = modelos_patologias.get(datos.patologia)
    if not modelo or len(datos.variables) != modelo.layer1.in_features:
        raise HTTPException(status_code=400, detail=f"Requiere {modelo.layer1.in_features if modelo else 'N/A'} variables.")
...
def procesar_triaje(datos: PeticionTriaje, token: str = Depends(verificar_token)):
    return analizar_sintomas_nlp(datos.sintomas_texto)
